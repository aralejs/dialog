...
var _araleEvents = require('arale-events');

var _araleEvents2 = _interopRequireDefault(_araleEvents);

var _araleTemplatable = require('arale-templatable');

var _araleTemplatable2 = _interopRequireDefault(_araleTemplatable);

var _araleMessenger = require('arale-messenger');
...
var Dialog = _araleOverlay2['default'].extend({
  ...
  events: {
    'click [data-role=close]': function clickDataRoleClose(e) {
      e.preventDefault();
      this.hide();
    }
  },
  ...
  _onRenderCloseTpl: function _onRenderCloseTpl(val) {
    if (val === '') {
  ...
  // ---
  // 绑定触发对话框出现的事件
  _setupTrigger: function _setupTrigger() {
  ...
  // 绑定元素聚焦状态
  _setupFocus: function _setupFocus() {
    this.after('show', function () {
      this.element.focus();
    });
  ...
  _syncHeight: function _syncHeight() {
    var h;
    // 如果未传 height，才会自动获取
    if (!this.get('height')) {
      try {
